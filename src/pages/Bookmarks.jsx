    ...
    return (
      <div className="flex items-center justify-center min-h-[60vh]">
        <div className="text-center max-w-sm">
          <div className="w-20 h-20 mx-auto mb-6 rounded-2xl bg-muted flex items-center justify-center"><Bookmark size={32} className="text-muted-foreground" /></div>
  ...
  return (
    <div className="max-w-3xl mx-auto">
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-2xl md:text-3xl font-serif font-semibold">Bookmarks <span className="ml-2 text-sm font-sans font-normal text-muted-foreground">({bookmarks.length})</span></h1>
      </div>
            ...
            <div className="flex items-start justify-between gap-4">
              <div className="flex-1 min-w-0">
                <div className="flex items-center flex-wrap gap-2 text-sm mb-1">
                  <Link to={`/bible/${bm.bookId}/${bm.chapter}`} className="font-medium hover:text-primary-600 transition-colors">{bm.bookName}</Link>
                  <span className="text-muted-foreground">{bm.chapter}:{bm.verse}</span>
                  <span className="text-xs text-muted-foreground flex items-center gap-1"><Calendar size={12} />{formatDate(bm.createdAt)}</span>
                </div>
                <p className="font-serif leading-relaxed">"{bm.text}"</p>
              </div>
            </div>
            <div className="flex items-center gap-1 mt-3 pt-3 border-t border-border">
              <Link to={`/bible/${bm.bookId}/${bm.chapter}`} className="flex items-center gap-1 px-3 py-1.5 rounded-lg hover:bg-muted transition-colors text-sm">Open <ArrowRight size={14} /></Link>
              <button onClick={() => handleCopy(`${bm.text} — ${bm.bookName} ${bm.chapter}:${bm.verse} (KJV)`)} className="p-1.5 rounded-lg hover:bg-muted transition-colors"><Copy size={16} /></button>
              <button onClick={() => handleShare(`${bm.text}
